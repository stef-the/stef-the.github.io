...
<main id="main" class="font-sans bg-gray-200 h-screen overflow-hidden">
	<div class="grid place-content-center h-screen">
		<div
			class="transition text-gray-800 hover:-translate-y-4 hover:bg-gray-800 hover:text-gray-200 grid place-content-center w-96 h-44 cursor-pointer rounded-md"
			on:click={() => {
                document.getElementById('main')?.classList.add('h-max')
                document.getElementById('main')?.classList.remove('h-screen')
				console.log('overflow turned on');
			}}
		>
			<h1 class="text-6xl cursor-inherit text-inherit pb-2 transition">{helloVar}</h1>
	...
	<div class="grid place-content-center h-screen">height</div>
</main>
